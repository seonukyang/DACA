...
k_S_score = silhouette_score(X_stand, Y_kmeans)
k_C_score = calinski_harabasz_score(X_stand, Y_kmeans)

print('계층적 군집분석 실루엣 계수 : ',k_S_score)
print('계층적 군집분석 CH 점수 : ',k_C_score)


X['result'] = ''
X['result'] = Y_kmeans

target_pred_k = kmeans.fit_predict(target_stand)

target['K-mean군집예측'] = ''
target['K-mean군집예측'] = target_pred_k
# target['DBSCAN군집예측'] = ''
# target['DBSCAN군집예측'] = target_pred_
X.to_csv('kmeans군집모델결과.csv', encoding='utf-8-sig')
# target.to_csv('kmenas군집분석결과.csv', encoding='utf-8-sig')
